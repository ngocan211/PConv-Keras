...
class PConvUnet(object):
    ...
    def __init__(self, img_rows=512, img_cols=512, vgg_weights="imagenet", inference_only=False, net_name='default',
                 gpus=1, vgg_device=None, lr=0.0002, train_bn=True):
        ...
        if self.gpus <= 1:
            self.model, inputs_mask = self.build_pconv_unet(train_bn=train_bn)
            self.compile_pconv_unet(self.model, inputs_mask, lr=lr)
        else:
