...
if __name__ == '__main__':
    ...
    if args.stage == 'tune' and not args.checkpoint:
    ...
    vgg_path = args.vgg_path if args.vgg_path else 'imagenet'
    vgg_device = args.vgg_device if args.vgg_device else None
    lr = 0.0002
    train_bn = True
    phase = 'train'

    model = PConvUnet(vgg_weights=vgg_path, vgg_device=vgg_device, lr=lr, train_bn=train_bn)
    ...
    if args.checkpoint:
        model.load(args.checkpoint, train_bn=train_bn, lr=lr)
    ...
    model.fit_generator(
        ...
        callbacks=[
            TensorBoard(
                log_dir=os.path.join(args.log_path, args.name + phase),
                write_graph=False
            ),
            ModelCheckpoint(
                os.path.join(args.log_path, args.name + phase, 'weights.{epoch:02d}-{loss:.2f}.h5'),
                monitor='val_loss',
                save_best_only=True,
